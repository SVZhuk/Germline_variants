...
def fastq_dir_to_samplesheet(
    fastq_dir,
    samplesheet_file,
    read1_extension="_L001_R1_001.fastq.gz",
    read2_extension="_L001_R2_001.fastq.gz",
    single_end=False,
    sanitise_name=True,
    ...
    if len(read_dict) > 0:
        ...
        with open(samplesheet_file, "w") as fout:
            header = ["patient", "sample", "lane", "fastq_1", "fastq_2"]
            ...
            for sample, reads in sorted(read_dict.items()):
                for idx, read_1 in enumerate(reads["R1"]):
                    ...
                    if idx < len(reads["R2"]):
                        read_2 = reads["R2"][idx]
                    sample_info = ",".join([sample, sample, "L001", read_1, read_2])
