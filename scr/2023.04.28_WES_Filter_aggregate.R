...
setwd("~/Desktop/AAK_lab/Variant_Annotation/data/2024.07.20_WES_reanalysis_part2")
...
file_ID <- "2024.07.20_WES_reanalysis_part2"
...
expr_table <- read.xlsx("~/Desktop/AAK_lab/Variant_Annotation/data/Annotation_DBs/2022.01.20-gene_expression_ranking.xlsx") %>% 
  select(description, rank_gse71613)

# Add expression rank for genes to reference table:
xref_table <- merge(xref_table, expr_table, by.x = "RefGene", by.y = "description", all.x = TRUE)
...
for (file in vep_files){
  ...
  subset <- comb_df_reorder %>%
    ...
    filter(gnomAD_exome_NFE < 0.001 | is.na(gnomAD_exome_NFE)) %>%
    filter(gnomAD_genome_NFE < 0.001 | is.na(gnomAD_genome_NFE)) %>%
    filter(TopMed_AF < 0.001 | is.na(TopMed_AF)) %>%
    filter(CADD >= 20 | is.na(CADD)) %>%
    filter(CoverageDepth >= 5) %>%
    #filter(case_when(type == 'SNP' ~ CoverageDepth >= 5,
    #                 type == 'INDEL' ~ CoverageDepth >= 10)) %>%
    filter(GATK_FILTER == "PASS") %>%
    filter(ClinVar_CLNSIG != "Benign") %>%
    filter(VEP_Impact == "HIGH" | VEP_Impact == "MODERATE") %>%
